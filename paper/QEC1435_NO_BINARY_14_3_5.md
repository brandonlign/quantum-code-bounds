...
```text
lambda >= 0,
c = lambda G + nu E >= 0 coefficientwise,
nu dot rhs < 0.
```
...
```text
0 <= c dot y = lambda dot (G y) + nu dot (E y)
              <= nu dot rhs < 0,
```
...
Suppose `p=Z_0 Z_1` belongs to `S`. The eight two-site Paulis commuting with `p` split into four complete cosets of `<p>` with physical two-site pattern
...
The resulting split-weight system has 39 variables and includes the coset inequality above, the low-weight distance equations and the signed-shadow constraints. Its exact Farkas certificate has
...
For each row, the exact certificate has nonnegative inequality multipliers, nonnegative column residuals and the negative normalization shown. The four necessary systems are therefore infeasible. Their multiplier data and independent integer checks appear in the computational supplement.
...
```text
lambda >= 0,
c = lambda G + nu E >= 0 on all 648 columns,
nu dot rhs = -125829120.
```

All 648 residual coefficients are nonnegative (403 positive and 245 zero). The disjoint `A_4=3` geometry is therefore impossible. The full multipliers and independent checks are in the computational supplement.
...
Grassl, Krotov, Sok, and Solé report 37 equivalence classes in the `(n,k)=(10,10)` cell of their additive `(n,2^k,>=5)_4` classification (Section 2.3, Table 1 of *The punctured dodecacode is unique*). Their paper reports the count, not the 37 generator matrices. We reconstruct the representatives using the described lengthening method and provide the resulting matrices and audit scripts in the repository.
...
The proof uses exact integer certificates for the necessary split-weight systems and an exhaustive classification of length-ten additive codes. The file `paper/COMPUTATIONAL_SUPPLEMENT.md` in the companion repository maps each claim to its verifier, certificate and data. The proof-code and data snapshot is archived at https://github.com/brandonlign/quantum-code-bounds/tree/ad8656883957975fc429895e64fbb497c480cba4 .

The standard replay checks the exact coefficient identities, Farkas multipliers, saved class representatives and Hall obstructions. The full lengthening census can also be regenerated, but is substantially more resource-intensive. Commands and dependencies are specified in the supplement and repository README. No floating-point feasibility result is used as a proof certificate.

## 10. AI-assisted tools and author responsibility

The author directed the research, evaluated the mathematical arguments and computational results, and reviewed the final manuscript. Generative-AI tools, including ChatGPT, Codex, and Claude, assisted with exploring proof strategies, developing and checking mathematical derivations, writing verification code, and drafting and editing the text. The author takes responsibility for the mathematical claims, computational results, references, and final manuscript.
...
* S. Ball, A. Centelles, and F. Huber, “Quantum error-correcting codes and their geometries,” *Annales de l'Institut Henri Poincare D: Combinatorics, Physics and Their Interactions* 10 (2023), no. 2, 337-405, DOI [10.4171/AIHPD/160](https://doi.org/10.4171/AIHPD/160). Research Problem 1 states that `[[14,3,5]]` is the smallest qubit stabilizer parameter set whose existence was unknown there.
* M. Grassl, D. Krotov, L. Sok, and P. Sole, “The punctured dodecacode is unique,” *Mathematics and Education in Mathematics* 55 (2026), 393-404, DOI [10.55630/mem.2026.55.393-404](https://doi.org/10.55630/mem.2026.55.393-404). Section 2.3 and Table 1 are the primary source for the published 37-class length-ten additive-code count and the lengthening method.
* M. Grassl, [Bounds on the minimum distance of additive quantum codes, n=14,k=3,q=4](https://codetables.de/QECC.php?k=3&n=14&q=4), maintained code table, accessed 21 September 2026. It records lower bound 4, upper bound 5, and the stored `[[14,3,4]]` construction (last-modified date shown there: 30 June 2005 for that entry).
* A. R. Calderbank, E. M. Rains, P. M. Shor, and N. J. A. Sloane, “Quantum error correction via codes over GF(4),” *IEEE Transactions on Information Theory* 44 (1998), 1369-1387, DOI [10.1109/18.681315](https://doi.org/10.1109/18.681315).
