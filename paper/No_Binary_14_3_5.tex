...
\(\left[\!\left[14,3,d\geq 5\right]\!\right]\) exists, settling the
...
minimum distance at least five, and symplectic hull dimension four. By
exhaustive lengthening, we reconstruct all 37 equivalence classes of
length-ten binary-additive codes of size 1,024 and minimum distance at
least five. Of these, 12 have symplectic hull dimension four; the other
25 do not, and the 12 violate the necessary coset-capacity bound. Together with the known
...
For use in the split-weight systems, write \(T_{a,b}=2048C_{a,b}\) and \(\mathtt{SH}_{a,b}=2048\Sh_{a,b}\); these are the integer transform numerators in the displayed formulas.
...
because every centralizer word in that weight range must be a stabilizer word when \(d(S) \geq 5\). This equality is valid even when \(S\) is impure.
...
Hence \(U_{j} \geq 0\), even without purity. Also \(A_{0}=1\), \(\sum_{j} A_{j}=2048\), and \(T_{j}=2048 A_{j}\) for \(j=1,2,3,4\).
...
E_{j} &=T_{j} -2048 A_{j},\qquad j=1,\dots,4, \\
F_{j} &=U_{j} -2048 \Sh_{j},\qquad j=0,\dots,3.
\end{aligned}
\]
...
Thus \(A_{2}=0\) for every hypothetical code. The repository README names both verification scripts and gives the standard replay command.
...
For each of the four non-disjoint rows of Table~\ref{tab:support-geometry}, let \(R\) be the rank-three subgroup generated by the three displayed Z-only words. Its cosets in the normalizer are grouped by prefix-weight histograms. The relaxations are summarized in Table~\ref{tab:rank-three}:
...
\captionof{table}{Farkas certificates excluding the four non-disjoint geometries of Table~\ref{tab:support-geometry}.}
...
For each of the three blocks and the two untouched sites, let \(y_{p,q,r,b}\) count complete \(R\)-cosets grouped by those literal patterns and suffix weight \(b\). There are \(6\cdot6\cdot6\cdot3=648\) nonnegative variables. The grouping forgets the actual coset labels, suffix letters, and global linearity, so it is a relaxation.
...
By the hypothesis, \(S \cap V_{4}=\langle p \rangle\), so projection of \(S/\langle p \rangle\) to \(W_{10}\) is injective. Let its image be a binary ten-space \(P \leq W_{10}\). Then there is a linear map \(T:P\to E\) such that
...
\[
\begin{aligned}
\operatorname{rank}(T)=6,\qquad \dim \operatorname{ker}(T)=10-6=4.
\end{aligned}
\]
...
\[
\begin{aligned}
\operatorname{rad}(P)=P \cap P^\perp_{W}=\operatorname{ker}(T),\qquad \dim \operatorname{rad}(P)=4.
\end{aligned}
\]
...
The recurrence starts from the zero code in the \((n,k)=(4,0)\) cell and builds additive codes one coordinate at a time. The projection argument shows that every code in the target class arises from one of the extension cases below.
...
Every zero-count predecessor cell listed above is computed and verified empty rather than treated as an absent cell. Hence Table~\ref{tab:predecessors} covers every predecessor needed by the recurrence through \((10,10)\).
...
Lemma~\ref{lem:ten-site} then forces a ten-site additive code of size 1,024, distance at least five, and hull dimension four. By Lemma 5, none of the 37 equivalence classes of length-ten additive codes of size 1,024 and distance at least five can supply the code required by Lemma 4: 25 have the wrong hull dimension, and the remaining 12 violate the coset-capacity bound. This is a contradiction.
\end{proof}
