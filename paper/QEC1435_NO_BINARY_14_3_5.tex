...
\usepackage{booktabs,longtable,tabularx,array,enumitem,microtype}
\usepackage{xcolor}
\usepackage{hyperref}
...
an integrality congruence to restrict low-weight stabilizer words. Exact
split-shadow infeasibility certificates then exclude weight-two words
and all configurations with three weight-four checks, leaving a unique
weight-four check.

That check yields a ten-coordinate binary-additive code of size 1,024,
...
violate a necessary 64-coset capacity bound. Together with the known
...
Throughout, \emph{physical} means weight or support on the original qubits, as opposed to a coordinate of an auxiliary relaxation. We use the qualifier when that distinction matters. The result applies to both pure and degenerate binary stabilizer codes, but not to arbitrary non-stabilizer quantum codes.
...
Thus the signed transform is not a formal signed polynomial. It is the weight enumerator of the affine coset \(v_{0}+S^\perp\); every split cell is a nonnegative integer.
...
Write \(Sh_j=\sum_{a+b=j}Sh_{a,b}\) for the total-weight count in the affine shadow. The quantities \(T_{j}/2048\) and \(U_{j}/2048\) are the actual total-weight counts in \(S^\perp\) and in the affine shadow. Hence \(U_{j} \ge 0\), even without purity. Also \(A_{0}=1\), \(\sum_{j} A_{j}=2048\), and \(T_{j}=2048 A_{j}\) for \(j=1,2,3,4\).
...
The remaining possibility \(A_{2}=1\) or \(2\) is removed by the physical weight-two subgroup certificate in Section 5.3.

\subsection{Integral affine-shadow parity}\label{integral-affine-shadow-parity}

The inequality alone does not determine whether \(A_{4}\) is zero. An integrality congruence supplies the missing condition. The coefficients below were chosen so that expansion of the MacWilliams and shadow equations leaves, modulo 4096, only the terms in \(A_{3}\) and \(A_{4}\). Let
...
The equations give \(L=2048\). Direct expansion gives the coefficients on \(A_{0},\dots,A_{14}\) as

\[
\begin{bmatrix}-4550656 \\-7348224 \\-1052672 \\-2623488 \\321536 \\-634880 \\221184 \\-28672 \\24576 \\24576 \\-45056 \\0 \\40960 \\-16384 \\-77824
\end{bmatrix}
\]

Modulo 4096 every coefficient vanishes except the coefficients of \(A_{3}\) and \(A_{4}\), each of which is 2048 modulo 4096. The coefficients of the four integer shadow variables are
...
Let \(R  \le  S\) be an isotropic subgroup supported on \(u\) of the original qubits, with the remaining \(f=14-u\) sites forming a suffix. Every element of \(S\) commutes with \(R\), so its prefix lies in \(R^\perp\). Also, \(S\) is a union of complete \(R\)-cosets.
...
\begin{aligned}
p(Q)=(p_{0},\dots,p_{u}), \\
p_{a} &=\#\{q \in Q : prefix physical weight(q)=a\}.
\end{aligned}
\]

Let \(y_{p,b}\) be the number of complete \(R\)-cosets selected by \(S\), aggregated over all suffix Pauli labels of physical weight \(b\), and grouped only by the prefix pattern \(p\). Then \(y_{p,b} \ge 0\) and
...
The transforms \(T_{a,b}\) and \(\mathtt{SH}_{a,b}\) use Pauli weights on the actual prefix and suffix sites. The capacity bound introduced in Section 6 is not used in these exclusions.
...
An exact certificate consists of nonnegative \(\lambda\), free-sign \(\nu\), and a coefficient vector \(c\) satisfying

\[
\begin{aligned}
\lambda  &\ge  0, \\
c &=\lambda G + \nu M  \ge  0 coefficientwise, \\
& \nu \cdot rhs < 0.
\end{aligned}
\]
...
Suppose \(p=Z_{0} Z_{1}\) belongs to \(S\). Every weight-two Pauli label is locally Clifford equivalent to this representative; in particular, Hadamards on sites 0 and 1 send \(X_0X_1\) to \(Z_0Z_1\). The eight two-site Paulis commuting with \(p\) split into four complete cosets of \(\langle p \rangle\) with physical two-site pattern

\[
\begin{aligned}
(1,0,1) \text{once},   (0,2,0) \text{once},   (0,0,2) \text{twice}.
\end{aligned}
\]
...
Use the 39 split counts \(H_{a,b}\), \(a=0,1,2\), \(b=0,\dots,12\), as the variables. This is a weaker relaxation than retaining the complete-coset variables \(y_{p,b}\) from Section~5.1: every physical code supplies these counts and satisfies \(H_{a,b}\ge0\) and \(H_{2,b}\ge H_{0,b}\), while further coset-pattern relations are discarded. The system also includes the low-weight distance equations and the signed-shadow constraints. Its exact Farkas certificate satisfies the sign conditions of Section~5.2 on all 39 columns and has

\[
\begin{aligned}
\nu \cdot rhs &=-2571108352.
\end{aligned}
\]

Thus \(A_{2}=0\) for every hypothetical code.

\subsection{Excluding overlapping low-weight checks}\label{excluding-overlapping-low-weight-checks}
...
\[
\begin{aligned}
(0,0,1,1) \text{twelve times}, \\
& (0,1,1,0) \text{three times}, \\
& (1,0,0,1) \text{once}.
\end{aligned}
\]

The exact dual satisfies the sign conditions of Section~5.2 on all 36 columns and has \(\nu \cdot rhs=-22528\). This also excludes a weight-three check, independently of Lemma~\ref{lem:low-weight}.
...
\begin{aligned}
(1,0,0,0,3) \text{once}, \\
& (0,0,2,0,2) \text{nine times}, \\
& (0,0,0,4,0) \text{six times}.
\end{aligned}
\]
...
The exact Farkas certificate satisfies the sign conditions of Section~5.2 on all 55 columns and has

\[
\begin{aligned}
\nu \cdot rhs &=-320017816092672.
\end{aligned}
\]
...
All three nonzero words in \(\langle p,q \rangle\) have weight four on five sites. The normalizer has 64 four-element cosets and nine prefix-weight patterns. The exact certificate satisfies the sign conditions of Section~5.2 on all 90 columns and has

\[
\begin{aligned}
\nu \cdot rhs &=-52501014528.
\end{aligned}
\]
...
The six-site normalizer has 256 four-element cosets and 13 prefix patterns. The corresponding 117-variable system has an exact Farkas certificate satisfying the sign conditions of Section~5.2 in every column, with \(\nu \cdot rhs=-8820736\).
...
The nonzero subgroup words have weights \(4,4,6\). Here the normalizer has 14 prefix patterns; the resulting 126-variable system has an exact Farkas certificate satisfying the sign conditions of Section~5.2 in every column, with \(\nu \cdot rhs=-22212608\). Both six-site exclusions are checked separately, with the complete cosets enumerated in each case.

\subsection{Support geometry of weight-four checks}\label{support-geometry-of-weight-four-checks}
...
If \(A_{4}=3\), the three checks are independent, because the sum of two has physical weight six or eight, not four. Their support sets are pairwise disjoint or meet in one site. Up to physical site permutation and reordering of the checks, there are exactly five support patterns:
...
{\small
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\linewidth}{@{}X>{\centering\arraybackslash}p{1.0cm}>{\centering\arraybackslash}p{1.4cm}>{\centering\arraybackslash}p{1.6cm}>{\centering\arraybackslash}p{1.9cm}>{\centering\arraybackslash}p{1.8cm}@{}}
\toprule
...
\begin{lemma}[Unique weight-four check]\label{lem:unique-check}
Every binary \(\left[\!\left[14,3,d\geq5\right]\!\right]\) stabilizer has exactly one stabilizer word of weight four.
\end{lemma}
\begin{proof}
...
Suppose \(S\leq V_{14}\) is isotropic of dimension 11, has \(d(S)\geq5\), and has a unique stabilizer word of weight four. Then there exists a binary subspace \(C\leq V_{10}\) of dimension 10, minimum Pauli weight at least five, and \(\dim(C\cap C^\perp)=4\).
\end{lemma}
\begin{proof}
...
\[
\begin{aligned}
p &=Z_{0} Z_{1} Z_{2} Z_{3},
\end{aligned}
\]

on the first four original qubits. Write \(V_{4}\) for their Pauli space and \(W_{10}\) for the remaining ten-qubit Pauli space, so \(V_{14}=V_{4} \oplus W_{10}\).
...
Since \(S \cap V_{4}=\langle p \rangle\), projection of \(S/\langle p \rangle\) to \(W_{10}\) is injective. Let its image be a binary ten-space \(P \le W_{10}\). Then there is a linear map \(T:P\to E\) such that
...
Suppose \(z \in E\) is orthogonal to \(\operatorname{im}(T)\). Choose a four-site representative \(v\) of \(z\) in \(p^\perp\). It commutes with \(p\) and with every element of the graph, hence \(v \in S^\perp\). Its original physical weight is at most four. Distance at least five forces \(v \in S\). But \(S \cap V_{4}=\langle p \rangle\), so \(z=0\). Thus \((\operatorname{im}(T))^\perp=0\). Nondegeneracy of \(E\) gives \(\operatorname{im}(T)=E\), and therefore
...
Then \(\dim C=10\), so \(\lvert C\rvert=1024\). If a nonzero \(c \in C\) had physical weight at most four, the suffix-only vector \((0,c)\) would be in \(S^\perp\) and distance would force it into \(S\); this would be either a nonzero stabilizer word of weight below four or a second weight-four word; both are excluded by Sections~4 and~5. Thus \(d(C) \ge 5\). Because \(C^\perp=P\),
...
No \(\operatorname{GF}(4)\)-linearity assumption is used.
...
For every suffix representative \(w\in\xi\), a prefix representative \(v\) of \(e(\xi)\) makes \(v+w\) commute with \(S\). Conversely, commutation forces this prefix class for a given suffix class. Thus the mixed centralizer fibre indexed by \(\xi\) has minimum physical weight \(d(\xi)+\operatorname{cost}(e(\xi))\). If \(\xi\ne0\), its suffix is nonzero, so no vector in this fibre is \(p\). Each vector in the fibre therefore has weight at least five: if it lies in \(S\), Sections~4--5 leave \(p\) as the only nonzero stabilizer word of weight at most four; otherwise its weight is at least \(d(S)\). Hence
\[
d(\xi)+\operatorname{cost}(e(\xi))\ge5
\qquad(\xi\ne0).
\]

Every compatible graph map induces a symplectic isomorphism \(P/J\to E\), equivalently an ordered symplectic basis of \(E\). Before enumerating the \(1{,}451,520\) such bases, the bijection \(\xi\mapsto e(\xi)\) and the cost distribution give the physical Hall capacities
...
The capacities are the numbers of nonzero prefix classes of cost at least 4, 3, and 2, respectively. The inequalities follow by counting under the bijection \(\xi\mapsto e(\xi)\); they are necessary Hall-type conditions for the required symplectic isomorphism. Failure excludes every physical symplectic lift; passing the inequalities does not establish that such a lift exists.
...
\subsection{Exact equivalence certificates}\label{exact-equivalence-certificates}

For every candidate the census constructs a colored incidence graph with one vertex for every codeword, three nonzero-symbol vertices for every coordinate, and one coordinate-group vertex binding each triple of symbols. The colors separate the three vertex types. A color-preserving graph isomorphism is exactly a codeword reorder, a coordinate permutation, and an arbitrary permutation of the three nonzero symbols in each coordinate. The latter is \(\operatorname{GL}(2,2)\). The final \texttt{pynauty} certificate is therefore an exact additive-monomial equivalence certificate; the weight and pair-rank invariants are only performance filters. The graph-isomorphism implementation is based on nauty {[}McKay and Piperno (2014){]}.

\subsection{Hull and physical Hall results}\label{hull-and-physical-hall-results}
...
\toprule
class & suffix weight spectrum \(\{w:N_w\}\) & \(N_{\leq1}\) & \(N_{\leq2}\) & \(N_{\leq3}\) & result \\
\midrule
...
In every row \(N_{ \le 3}\) is 62 or 63, exceeding its capacity 59 (and several rows already fail at \(N_{ \le 2}>32\)).

\begin{lemma}[Additive lift exclusion]\label{lem:additive-lift}
None of the 37 length-ten classes can supply the code in Lemma~\ref{lem:ten-site}: 25 have a different hull dimension, and the other twelve violate the necessary Hall capacity bound.
\end{lemma}
\begin{proof}
...
Assume a binary \(\left[\!\left[14,3,d\ge 5\right]\!\right]\) stabilizer exists. Sections~3 and~4 give \(A_{1}=A_{2}=A_{3}=0\), \(A_{4} \in \{1,3\}\). Sections~5.3--5.5 show that all overlapping weight-four support pairs are physically impossible and classify the three-check case into five support geometries. Section~5.6 excludes the four overlapping rank-three geometries, and Section~5.7 excludes the disjoint \(4+4+4+2\) geometry. Hence \(A_{4}=1\).

Lemma~\ref{lem:ten-site} forces a ten-site additive code of size 1,024, distance at least five, and hull dimension four. Section 7 exhausts the 37 equivalence classes of such ten-site additive codes and rejects every one: 25 by hull dimension and 12 by the physical Hall capacity. This is a contradiction.
\end{proof}

The proof is specific to these parameters and to the stabilizer family; it gives no bound at other lengths and no conclusion for non-stabilizer codes.

\section{Computational material}\label{computational-material}

The proof uses exact integer certificates for the necessary split-weight systems and an exhaustive classification of length-ten additive codes. The default replay checks the coefficient identities, Farkas multipliers, physical coset models, saved class representatives and Hall obstructions. It uses the committed representatives; the full lengthening census is a separate, more resource-intensive run. The archived package contains the exact certificate data, generator matrices, verification programs and concise reproduction instructions. No floating-point feasibility result is used as a proof certificate. The complete computational record is in the \mbox{\href{https://github.com/brandonlign/quantum-code-bounds/archive/c14d6dadf2c5f902d9c149ed992fbac54b63596e.zip}{immutable code archive}}.
