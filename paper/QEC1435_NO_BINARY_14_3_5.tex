...
\begin{aligned}
p(Q)=(p_{0},\dots,p_{u}), \\
p_{a} &=\#\{q \in Q : \operatorname{wt}_{\mathrm{pref}}(q)=a\}.
\end{aligned}
\]
...
Let \(p\) be the unique weight-four stabilizer word. After physical local Clifford conjugation and a site permutation, assume \(p=Z_{0}Z_{1}Z_{2}Z_{3}\) on the first four original qubits. Write \(V_{4}\) for their Pauli space and \(W_{10}\) for the remaining ten-qubit Pauli space, so \(V_{14}=V_{4} \oplus W_{10}\).
...
Every compatible graph map induces a symplectic isomorphism \(P/J\to E\). The bijection \(\xi\mapsto e(\xi)\) and the cost distribution give the following necessary capacities without enumerating these isomorphisms:
...
The 25 classes with hull dimension different from four cannot arise from the code \(C\) required by Lemma~\ref{lem:ten-site}. For the 12 hull-four classes, the exact physical quotient audit gives:
...
Assume a binary \(\left[\!\left[14,3,d\ge 5\right]\!\right]\) stabilizer exists. Sections~3--4 give \(A_{1}=A_{3}=0\), \(A_{2}\leq2\), and \(A_{4}\in\{1,3\}\). Section~5.3 then excludes weight-two stabilizers, so \(A_{2}=0\). Sections~5.3--5.5 exclude all pairs of distinct weight-four checks whose supports intersect in at least two sites and classify the three-check case into five support geometries. Section~5.6 excludes the four overlapping rank-three geometries, and Section~5.7 excludes the disjoint \(4+4+4+2\) geometry. Hence \(A_{4}=1\).
...
The proof uses exact integer certificates for the split-weight systems and an exhaustive classification of length-ten additive codes. The verification scripts check the coefficient identities, Farkas multipliers, physical coset models, saved class representatives, and Hall obstructions. The default verification uses the saved representatives rather than rerunning the more computationally expensive lengthening census. No floating-point feasibility calculation serves as a certificate. The certificate data, generator matrices, verification scripts, and reproduction instructions are available in the \href{https://github.com/brandonlign/quantum-code-bounds}{project repository}.
