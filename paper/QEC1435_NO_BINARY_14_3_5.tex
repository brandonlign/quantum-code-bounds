...
We first derive relaxations that are necessary for any hypothetical code containing a specified subgroup. Exact infeasibility certificates then exclude the remaining low-weight configurations.
...
The transforms \(T_{a,b}\) and \(\mathtt{SH}_{a,b}\) use Pauli weights on the actual prefix and suffix sites. The coset-capacity bound of Section~6.2 is not used in these exclusions.
...
Suppose \(p=Z_{0} Z_{1}\) belongs to \(S\). A one-qubit Clifford acts transitively on the three nonidentity Pauli labels, so independent local Clifford conjugations on sites 0 and 1 send any weight-two stabilizer word to this representative while preserving weight and the code parameters. In particular, Hadamards send \(X_0X_1\) to \(Z_0Z_1\). The eight two-site Pauli labels commuting with \(p\) split into four complete cosets of \(\langle p \rangle\) with physical two-site pattern
...
Every hypothetical code satisfies the following necessary constraints: \(H_{0,0}=1\), \(\sum_{a,b}H_{a,b}=2048\), and \(H_{a,b}\geq0\); the 11 equalities \(T_{a,b}=2048H_{a,b}\) for \(1\leq a+b\leq4\); the 13 coset inequalities \(H_{2,b}\geq H_{0,b}\); and, for all 39 split cells, centralizer inclusion \(T_{a,b}\geq2048H_{a,b}\), centralizer nonnegativity \(T_{a,b}\geq0\), and affine-shadow nonnegativity \(\mathtt{SH}_{a,b}\geq0\). Together with the two normalization equalities, these give 13 equality rows; the listed inequalities give 130 rows, in addition to the 39 nonnegativity conditions on the variables. These constraints form a necessary relaxation; it discards the further coset-pattern relations in Section~5.1.

The exact Farkas certificate has 13 nonnegative inequality multipliers and 10 nonzero equality multipliers. The residual vector \(c\) has 17 positive and 22 zero entries and satisfies the 39 column identities \(c=\lambda G+\nu M\). Its normalization is
...
The exact multipliers and two independent integer-arithmetic verifiers are identified in the verification instructions cited in Section~\ref{computational-material}.
...
The exact dual satisfies the sign conditions of Section~5.2 on all 36 columns and has \(\nu \cdot rhs=-22528\). This also excludes a weight-three stabilizer word, independently of Lemma~\ref{lem:low-weight}.
...
The omitted case \(t=4,h=0\) would give the same Pauli word twice, not two distinct stabilizer words. Thus two distinct weight-four stabilizer words meet in at most one qubit. If they meet in one, commutation forces the same local Pauli letter there. Independent local Clifford transformations therefore map all weight-four stabilizer words to Z-only words simultaneously. This does not imply that the full stabilizer is CSS.

\begin{minipage}{\linewidth}
If \(A_{4}=3\), the three weight-four stabilizer words are independent: the sum of any two has physical weight six or eight, not four. Their support sets are pairwise disjoint or meet in one site. Up to physical site permutation and reordering, there are exactly five support patterns:
...
For each of the four non-disjoint rows in the table, let \(R\) be the physical rank-three subgroup generated by the three displayed Z-only words. Its complete cosets in the original-site normalizer are enumerated and grouped by their prefix-weight histograms. The exact relaxations are:
...
For every suffix representative \(w\in\xi\), a prefix representative \(v\) of \(e(\xi)\) makes \(v+w\) commute with \(S\). Conversely, commutation forces this prefix class for a given suffix class. Thus the mixed centralizer fiber indexed by \(\xi\) has minimum physical weight \(d(\xi)+\operatorname{cost}(e(\xi))\). If \(\xi\ne0\), its suffix is nonzero, so no vector in this fiber is \(p\). Each vector in the fiber therefore has weight at least five: if it lies in \(S\), Sections~4--5 leave \(p\) as the only nonzero stabilizer word of weight at most four; otherwise its weight is at least \(d(S)\). Hence
\[
...
The coset-capacity bound consists of these three inequalities. The capacities are the numbers of nonzero prefix classes of cost at least 4, 3, and 2, respectively. Counting under the bijection \(\xi\mapsto e(\xi)\) gives Hall-type necessary conditions for the required symplectic isomorphism. Failure excludes every physical symplectic lift; satisfying the bound does not establish that a lift exists.
...
The threshold four is exact: a word with nonzero new symbol gains one physical unit of weight, while a word with zero new symbol lies in the distance-five parent. Thus every target code appears in one of these three cases.
...
The enumeration produces 37 generator matrices, each defining 1,024 words of minimum distance five. Independent exact audits recompute their ranks, distances, hull dimensions, and coset minima. The hull dimensions are
...
The 25 classes with hull dimension different from four cannot arise from the code \(C\) required by Lemma~\ref{lem:ten-site}. For the 12 hull-four classes, the exact physical coset-capacity audit gives:
...
The verification uses exact integer certificates for the split-weight systems and an exhaustive classification of length-ten additive codes. The standard replay requires Python 3.10 or newer and Node.js. From the repository root, run
\begin{center}
\texttt{bash experiments/qec1435\_replay\_nonexistence.sh}
\end{center}
This replay checks the exact arithmetic and the committed class representatives but does not regenerate the classification. The optional \texttt{--census} mode reconstructs the classification and requires the pinned \texttt{pynauty==2.8.8.1} dependency. The pin and detailed verification instructions are included in the repository. Replay outputs are temporary and do not replace the committed representatives or audit data.
